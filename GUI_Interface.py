...
def showabout():
    compiled_date = f'Complied on {COMPILE_DATE}.\nDetected time: {currentdate.strftime("%Y-%m-%d")}' if EXPIRY_DAYS else ''
    messagebox.showinfo('Author', '{}Written by Qingze (Eric) Li\n\tMay 22 2023.'.format(f"{compiled_date}\n" if compiled_date else ''))
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        customtkinter.CTkLabel(self.fr0, text='Select mutual fund:', font=customtkinter.CTkFont(size=20, weight="bold")).grid(row=0, column=0, sticky=W, padx=(0,30))
        ...
        customtkinter.CTkLabel(self.fr0, text="{} Edition".format(f"{EXPIRY_DAYS} days" if EXPIRY_DAYS else "Unlimited")).grid(row=0, column=2, sticky=W, padx=(0,10))
        customtkinter.CTkButton(self.fr0, command=showabout, text='About', width=44, height=18).grid(row=0, column=3, sticky=W)
        ...
        self.radio_var = StringVar(value=radio_options[0][0]) # Default selected radiobutton is the first mutual fund.
        self.fr1.grid(padx=10, row=1, sticky="nsew", pady=(4, 0))
        for row in range(rows):
            for column in range(len(radio_options[row])): # The last row could have less items then columns so can't use range(columns)
                ...
                Gradioseelection(self.fr1, self.radiobutton_params).grid(row=row, column=column, padx=15, pady=(8,0), sticky=W)
        ...
        # when the size of the parent container changes. 3 column will share the available space proportionally based on their weights.
...
if __name__ == '__main__':
    ...
    EXPIRY_DAYS = 300 # 0 stands for unlimited access. Update to postive int to setup expiration date from compile date.
    if EXPIRY_DAYS:
    ...
    #radio_state = NORMAL if show_mutual_fund_history_graph.sanity_check()['Success'] else DISABLED
